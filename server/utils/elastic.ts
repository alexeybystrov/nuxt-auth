...
export const postSyncToElastic = async (post: Post) => {
  try {
    ...
    // Silently fail if Elasticsearch is not configured
    if (error.message?.includes('Elasticsearch is not configured')) {
      console.warn('Elasticsearch not available, skipping post sync');
      return;
    }
    throw error;
  }
...
export const bulkSyncPosts = async (posts: Post[]) => {
  try {
    ...
    // Silently fail if Elasticsearch is not configured
    if (error.message?.includes('Elasticsearch is not configured')) {
      console.warn('Elasticsearch not available, skipping bulk sync');
      return;
    }
    throw error;
  }
